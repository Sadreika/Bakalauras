namespace Fly540
{
    public class Regexes
    {
        ...
        public static string FareFamilyCurrencyPriceSeatsSoldOut = @"(?:header\s*class-[0-9]+\u0022>(.*?)<).*(?:(?:class=\u0022pkgprice\u0022>([a-z]{3})\s*([0-9,]+).*(?:(?:Only\s*([0-9]+)\s*seat)|(?:</button>)))|(SOLD\s*OUT))";
        ...
        public static string OutboundAndInboundPriceAndTaxes = @"[A-Z]{3}\s*-\s*[A-Z]{3}.*?num\u0022>([0-9,.]+).*?num\u0022>([0-9,.]+)";
    }
}
